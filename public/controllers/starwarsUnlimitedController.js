document.addEventListener('DOMContentLoaded', () => {
    window.initializeSWUController = function (canManage) {
        ...
        function renderTournamentsList(filterLigaId) {
            const container = $id('lista-torneos-swu');
            if (!container) return;
            const showFinalized = $id('filter-show-finalized-swu')?.checked || false;
            ...
            container.innerHTML = filtered.map(t => {
                ...
                if (canManage) {
                    if (t.estado === 'borrador') {
                        buttons += `
                            <button class="btn btn-sm btn-outline-primary btn-add-players-torneo-swu" data-id="${t.id}"><i class="fa-solid fa-users-gear"></i> Jugadores</button>
                            <button class="btn btn-sm btn-success btn-iniciar-swu" data-id="${t.id}"><i class="fa-solid fa-play"></i> Iniciar</button>
                            <button class="btn btn-sm btn-outline-secondary btn-editar-torneo-swu" data-id="${t.id}"><i class="fa-solid fa-edit"></i></button>
                        `;
                    } else if (t.estado === 'en_curso') {
                        buttons += `<button class="btn btn-sm btn-primary btn-gestionar-swu" data-id="${t.id}"><i class="fa-solid fa-gears"></i> Gestionar</button>`;
                    } else {
                        buttons += `<button class="btn btn-sm btn-outline-info btn-ver-detalles-torneo-swu" data-id="${t.id}"><i class="fa-solid fa-eye"></i> Resultados</button>`;
                    }
                    buttons += `<button class="btn btn-sm btn-outline-danger btn-eliminar-torneo-swu" data-id="${t.id}"><i class="fa-solid fa-trash"></i></button>`;
                } else {
                    if (t.estado === 'en_curso') {
                        buttons += `<button class="btn btn-sm btn-primary btn-gestionar-swu" data-id="${t.id}"><i class="fa-solid fa-eye"></i> Ver Rondas</button>`;
                    } else if (t.estado === 'finalizado') {
                        buttons += `<button class="btn btn-sm btn-outline-info btn-ver-detalles-torneo-swu" data-id="${t.id}"><i class="fa-solid fa-eye"></i> Clasificación</button>`;
                    } else {
...
function calculateStandings(torneo) {
            const pts = {};
            const jugMap = {};
            (torneo.jugadores || []).forEach(j => { jugMap[j.id] = j.nombre; pts[j.id] = { puntos: 0, progresivo: 0, bh: 0, rivales: [], dropped: j.dropped || false }; });
            (torneo.rondas || []).forEach(r => {
                (r.mesas || []).forEach(m => {
                    if (m.bye) {
                        if (!pts[m.bye]) pts[m.bye] = { puntos: 0, progresivo: 0, bh: 0, rivales: [], dropped: false };
                        pts[m.bye].puntos += PT_BYE;
                        return;
                    }
                    const [p1, p2] = m.jugadores;
                    if (!pts[p1]) pts[p1] = { puntos: 0, progresivo: 0, bh: 0, rivales: [], dropped: false };
                    if (!pts[p2]) pts[p2] = { puntos: 0, progresivo: 0, bh: 0, rivales: [], dropped: false };
            ...
            return Object.entries(pts).map(([id, data]) => ({
                id, nombre: jugMap[id] || '?',
                puntos: data.puntos, progresivo: data.progresivo, bh: data.bh, rivales: data.rivales, dropped: data.dropped || false,
            })).sort((a, b) => b.puntos - a.puntos || b.progresivo - a.progresivo || b.bh - a.bh);
        }

        function renderStandings(torneo) {
            const tbody = $id('tabla-clasificacion-swu');
            if (!tbody) return;
            ...
            tbody.innerHTML = list.map((r, i) => `
                <tr${r.dropped ? ' class="table-secondary"' : ''} style="${r.dropped ? 'opacity:0.6;' : ''}"><td class="fw-bold">${i + 1}</td>
                    <td>${escapeHtml(r.nombre)} ${r.dropped ? '<span class="badge bg-danger ms-1">retirado</span>' : ''}</td>
...
`).join('');
        ...
        function poblarSelectOmitirTop() {
            const sel = $id('sorteo-omitir-top-swu');
            if (!sel) return;
            const torneo = selectedTournament;
            const total = (torneo?.jugadores || []).length;
        ...
        function lanzarSorteo() {
            const torneo = selectedTournament;
            const resultEl = $id('sorteo-resultado-swu');
            if (!torneo || !resultEl) return;
            const jugadores = torneo.jugadores || [];
            if (jugadores.length === 0) { showAlert('No hay jugadores en el torneo.', 'warning'); return; }

            const omitirTop = parseInt($id('sorteo-omitir-top-swu')?.value || '0', 10);
            let num = parseInt($id('sorteo-numero-swu')?.value || '1', 10);
            if (Number.isNaN(num) || num < 1) num = 1;
            const contarRetirados = $id('sorteo-retirados-swu')?.checked || false;

            let pool = calculateStandings(torneo);
            if (omitirTop >= pool.length) { showAlert('"Omitir top" excluye a todos los jugadores. Bájalo para poder sortear.', 'warning'); return; }
            pool = pool.slice(omitirTop);
            if (!contarRetirados) pool = pool.filter(s => !s.dropped);
            if (pool.length === 0) { showAlert('No quedan jugadores elegibles (revisa "Omitir top" y los retirados).', 'warning'); return; }

            if (num > pool.length) {
                showAlert(`Solo hay ${pool.length} jugador(es) elegible(s); se sortearán ${pool.length}.`, 'info');
                num = pool.length;
            }
...
async function handleGenerateRound() {
            ...
            if (activos.length < 2) { showAlert('Se necesitan al menos 2 jugadores activos.', 'warning'); return; }

            const rondas = torneo.rondas || [];
            const pairings = generatePairings(activos, rondas);
        ...
        function openTournamentManagement(torneoId) {
            ...
            $id('lista-torneos-swu-container').style.display = 'none';
            $id('gestion-torneo-swu-container').style.display = 'block';
            $id('gt-swu-nombre').textContent = torneo.nombre || '';
            ...
            $id('gt-swu-detalles').textContent = `${fechaStr}${asociacionTxt ? ' | ' + asociacionTxt : ''} | Formato: Modern`;
            const badge = $id('gt-swu-status');
            ...
            const btnIniciar = $id('btn-iniciar-torneo-swu-gestion');
            ...
            $id('btn-generar-ronda-swu').style.display = canManage && !isFinalized && !isBorrador && !hasTopCut ? 'inline-block' : 'none';
            $id('btn-deshacer-ronda-swu').style.display = canManage && !isFinalized && hasRondas && !hasTopCut ? 'inline-block' : 'none';
            $id('btn-finalizar-torneo-swu').style.display = canManage && !isFinalized && hasRondas ? 'inline-block' : 'none';
            $id('btn-deshacer-topcut-swu').style.display = canManage && !isFinalized && hasTopCut ? 'inline-block' : 'none';
...
function renderJugadoresList(torneo) {
            const ul = $id('gt-swu-jugadores');
            if (!ul) return;
            const jugs = torneo.jugadores || [];
            $id('gt-swu-count').textContent = jugs.length;
            if (jugs.length === 0) { ul.innerHTML = '<li class="list-group-item text-muted">Sin jugadores</li>'; return; }
            const gestEnCurso = canManage && torneo.estado === 'en_curso' && !torneo.topCut;
            const pendientes = tieneResultadosPendientes(torneo);
            ul.innerHTML = jugs.map(j => {
                const isDropped = !!j.dropped;
                const bloqueado = isDropped && isPlayerBloqueado(j, torneo);
                let dropControls = '';
                if (gestEnCurso) {
                    if (isDropped) {
                        if (bloqueado) {
                ...
                <li class="list-group-item d-flex justify-content-between align-items-center ${isDropped ? 'list-group-item-secondary' : ''}" style="${isDropped ? 'opacity:0.65;' : ''}">
                    <div class="d-flex align-items-center gap-2 flex-wrap">
                        ...
                        ${isDropped ? '<span class="badge bg-danger">retirado</span>' : ''}
                        ${j.id.startsWith('invitado_') ? '<span class="badge bg-secondary">Invitado</span>' : '<span class="badge bg-primary">Registrado</span>'}
                    </div>
                    <div class="d-flex align-items-center gap-2">${dropControls}</div>
                </li>
            `;
            }).join('');
        }
        ...
        function tieneResultadosPendientes(torneo) {
            const rondas = (torneo || selectedTournament).rondas || [];
            if (rondas.length === 0) return false;
            const ultima = rondas[rondas.length - 1];
            return (ultima.mesas || []).some(m => {
                if (m.bye) return false;
                return !m.ganador && !m.empate;
            });
        }

        function isPlayerBloqueado(jugador, torneo) {
            return !!(jugador.dropped && ((torneo || selectedTournament).rondas || []).length > (jugador.droppedAtRound || 0));
        }

        async function handleDropJugador(playerId) {
            if (!selectedTournament) return;
            const torneo = selectedTournament;
            const jugador = (torneo.jugadores || []).find(j => j.id === playerId);
            if (!jugador) return;

            if (torneo.topCut) { showAlert('No se puede retirar a un jugador con el top cut activo.', 'warning'); return; }
            if (tieneResultadosPendientes(torneo)) { showAlert('Completa los resultados de la última ronda antes de retirar a un jugador.', 'warning'); return; }

            const st = calculateStandings(torneo).find(s => s.id === playerId);
            const puntos = st ? st.puntos : 0;
            if (!confirm(`¿Retirar a "${jugador.nombre}" del torneo? Conservará sus ${puntos} punto(s) actuales, pero ya no se emparejará en rondas futuras.`)) return;

            try {
                const jugadores = (torneo.jugadores || []).map(j =>
                    j.id === playerId ? { ...j, dropped: true, droppedAtRound: (torneo.rondas || []).length } : j
                );
                await db.collection('torneos').doc(torneo.id).update({ jugadores });
                await loadInitialData();
                const updated = torneosCache.find(t => t.id === torneo.id);
                if (updated) { selectedTournament = updated; openTournamentManagement(updated.id); }
                showAlert(`"${jugador.nombre}" retirado del torneo. Conserva ${puntos} punto(s).`, 'warning');
        ...
        async function handleReincorporarJugador(playerId) {
            ...
            if (isPlayerBloqueado(jugador, torneo)) { showAlert(`"${jugador.nombre}" está bloqueado porque ya se generó una ronda posterior a su retirada. Deshaz esa ronda para poder reincorporarlo.`, 'warning'); return; }
            if (!confirm(`¿Reincorporar a "${jugador.nombre}" al torneo? Volverá a contar en los emparejamientos.`)) return;
            ...
            try {
                ...
                const updated = torneosCache.find(t => t.id === torneo.id);
                if (updated) { selectedTournament = updated; openTournamentManagement(updated.id); }
                showAlert(`"${jugador.nombre}" reincorporado al torneo.`, 'success');
            } catch (e) { showAlert('Error: ' + e.message, 'danger'); }
        }
                ...
                <div class="bg-white p-3 mb-4 rounded-3 border-0 shadow-sm">
                    ...
                    ${r.mesas.map(m => {
                        ...
                        return `<div class="match-card p-2 mb-1 d-flex justify-content-between align-items-center">
                            ...
                            ${canManage && !resultadosBloqueados ? `<button class="btn btn-sm btn-outline-primary btn-resultados-swu" data-ronda="${r.numero}" data-mesa="${m.numero}"><i class="fa-solid fa-table-tennis"></i></button>` : ''}
                        </div>`;
                    }).join('')}
                </div>
            `).join('');
                    ...
                    container.innerHTML = `<div class="text-center py-3">
                        ...
                        ${canManage ? `
                            <div class="d-flex justify-content-center gap-2 align-items-center flex-wrap">
                                ...
                                <button class="btn btn-sm btn-outline-primary" id="btn-generar-topcut-swu">Generar Top Cut</button>
                            </div>` : ''}
                    </div>`;
                } else {
                        ...
                        <div class="bracket-col">
                            ...
                            ${br.mesas.map(m => {
                                ...
                                return `<div class="bracket-match ${done ? 'border-success' : ''}">
                                    ...
                                    ${canManage && !done && torneo.estado !== 'finalizado' ? `<button class="btn btn-sm btn-outline-primary mt-1 btn-resultados-topcut-swu" data-ronda="${bi}" data-mesa="${m.numero}"><i class="fa-solid fa-table-tennis"></i></button>` : ''}
                                </div>`;
                            }).join('')}
                        </div>
                    `).join('')}
                </div>
                ${canShowAvanzar ? `<div class="text-center mt-3"><button class="btn btn-primary" id="btn-avanzar-topcut-swu"><i class="fa-solid fa-forward"></i> Avanzar a siguiente ronda</button></div>` : ''}`;
        }
        ...
        function openMesaResultsModal(rondaNum, mesaNum, isTopCut = false) {
            ...
            $id('modal-res-swu-mesa').textContent = `${rondaNum}.${mesaNum}`;
            const body = $id('modal-resultados-swu-body');
            ...
            body.innerHTML = `
                <input type="hidden" id="res-swu-ronda" value="${rondaNum}">
                <input type="hidden" id="res-swu-mesa" value="${mesaNum}">
                <input type="hidden" id="res-swu-topcut" value="${isTopCut ? '1' : '0'}">

                <div class="text-center mb-3">
                    <label class="fw-bold d-block mb-2">${n1}</label>
                    <div class="btn-group" role="group">
                        <input type="radio" class="btn-check" name="res-swu-result" id="res-p1_2-0" value="p1_w2" autocomplete="off" ${prevResult === 'p1_w2' ? 'checked' : ''}>
                        <label class="btn btn-outline-success btn-sm" for="res-p1_2-0" title="Gana 2-0">2-0</label>
                        <input type="radio" class="btn-check" name="res-swu-result" id="res-p1_2-1" value="p1_w21" autocomplete="off" ${prevResult === 'p1_w21' ? 'checked' : ''}>
                        <label class="btn btn-outline-success btn-sm" for="res-p1_2-1" title="Gana 2-1">2-1</label>
                        <input type="radio" class="btn-check" name="res-swu-result" id="res-p1_1-0" value="p1_w1" autocomplete="off" ${prevResult === 'p1_w1' ? 'checked' : ''}>
                ...
                <div class="text-center mb-3">
                    <div class="btn-group" role="group">
                        <input type="radio" class="btn-check" name="res-swu-result" id="res-draw" value="draw" autocomplete="off" ${prevResult === 'draw' ? 'checked' : ''}>
                ...
                <div class="text-center mb-3">
                    ...
                    <div class="btn-group" role="group">
                        <input type="radio" class="btn-check" name="res-swu-result" id="res-p2_1-0" value="p2_w1" autocomplete="off" ${prevResult === 'p2_w1' ? 'checked' : ''}>
                        <label class="btn btn-outline-success btn-sm" for="res-p2_1-0" title="Gana 1-0 (tiempo)">1-0</label>
                        <input type="radio" class="btn-check" name="res-swu-result" id="res-p2_2-1" value="p2_w21" autocomplete="off" ${prevResult === 'p2_w21' ? 'checked' : ''}>
                        <label class="btn btn-outline-success btn-sm" for="res-p2_2-1" title="Gana 2-1">2-1</label>
                        <input type="radio" class="btn-check" name="res-swu-result" id="res-p2_2-0" value="p2_w2" autocomplete="off" ${prevResult === 'p2_w2' ? 'checked' : ''}>
            ...
            `;
            const modal = new bootstrap.Modal($id('modal-resultados-swu'));
            modal.show();
        }
        ...
        async function handleResultadosSubmit(e) {
            e.preventDefault();
            if (!selectedTournament) return;
            const rondaNum = parseInt($id('res-swu-ronda').value);
            const mesaNum = parseInt($id('res-swu-mesa').value);
            const isTopCut = $id('res-swu-topcut').value === '1';

            const selectedResult = document.querySelector('input[name="res-swu-result"]:checked')?.value;
            ...
            try {
                ...
                bootstrap.Modal.getInstance($id('modal-resultados-swu'))?.hide();
                await loadInitialData();
                const updated = torneosCache.find(t => t.id === torneo.id);
                if (updated) { selectedTournament = updated; openTournamentManagement(updated.id); }
        ...
        function updateImportStatus() {
            const el = document.getElementById('import-status-swu');
            if (!el) return;
        ...
        function openJugadoresModal(torneoId) {
            ...
            const fActuales = document.getElementById('filter-jugadores-actuales-swu');
            if (fActuales) fActuales.value = '';
            const fPrevios = document.getElementById('filter-jugadores-previos-swu');
            if (fPrevios) fPrevios.value = '';
            const modal = new bootstrap.Modal($id('modal-jugadores-swu'));
            modal.show();
        }

        function renderTempJugadoresList() {
            const ul = $id('lista-jugadores-swu');
            if (!ul) return;
            const filterInput = document.getElementById('filter-jugadores-actuales-swu');
            ...
            $id('count-jugadores-swu').textContent = tempJugadores.length;
            ul.innerHTML = filtered.map((j, i) => {
                ...
                <li class="list-group-item d-flex justify-content-between align-items-center">
                    ...
                    <button class="btn btn-sm btn-outline-danger btn-quitar-jugador-swu" data-id="${j.id}"><i class="fa-solid fa-times"></i></button>
                </li>`;
            }).join('');
            updateImportStatus();
        }
        ...
        function handleAgregarUsuario() {
            const input = $id('input-buscar-usuario-swu');
        ...
        function handleAgregarInvitado() {
            const nombre = $id('input-nombre-invitado-swu')?.value.trim();
            ...
            if (!esLiga) {
                ...
                $id('input-nombre-invitado-swu').value = '';
                renderTempJugadoresList();
                return;
            }
            let codigo = $id('input-codigo-invitado-swu')?.value.trim().toUpperCase();
            ...
            $id('input-nombre-invitado-swu').value = '';
            $id('input-codigo-invitado-swu').value = '';
            renderTempJugadoresList();
        }
        ...
        function updateImportarInscritosButton() {
            const btn = document.getElementById('btn-importar-inscritos-swu');
            if (btn) {
        ...
        function renderJugadoresPrevios() {
            const tbody = document.getElementById('lista-jugadores-previos-swu');
            const paginationNav = document.getElementById('pagination-jugadores-previos-swu');
            const filterInput = document.getElementById('filter-jugadores-previos-swu');
            if (!tbody) return;
            const term = filterInput ? filterInput.value.trim().toLowerCase() : '';
            const filtered = term
        ...
        async function handleConfirmJugadores() {
            ...
            try {
                ...
                if (updated) { selectedTournament = updated; openTournamentManagement(updated.id); }
                bootstrap.Modal.getInstance($id('modal-jugadores-swu'))?.hide();
        ...
        function openTournamentModal(torneoId = null) {
            $id('form-torneo-swu').reset();
            $id('torneo-swu-id').value = '';
            $id('modal-torneo-swu-title').textContent = 'Crear Torneo Modern';
            $id('liga-bloque-swu').style.display = 'none';
            $id('independiente-bloque-swu').style.display = 'block';
            $id('torneo-swu-es-liga').checked = false;

            if (torneoId) {
                const t = torneosCache.find(x => x.id === torneoId);
                if (t) {
                    $id('modal-torneo-swu-title').textContent = 'Editar Torneo Modern';
                    $id('torneo-swu-id').value = t.id;
                    $id('torneo-swu-nombre').value = t.nombre || '';
                    $id('torneo-swu-fecha').value = (t.fecha && t.hora) ? `${t.fecha}T${t.hora}` : (t.fecha || '');
                    $id('torneo-swu-es-liga').checked = t.esJornadaLiga || false;
                    $id('liga-bloque-swu').style.display = t.esJornadaLiga ? 'block' : 'none';
                    $id('independiente-bloque-swu').style.display = t.esJornadaLiga ? 'none' : 'block';
                    if (t.esJornadaLiga) {
                        $id('select-torneo-liga-swu').value = t.ligaId || '';
                        handleTorneoLigaChange(t.ligaId);
                    } else {
                        $id('select-torneo-evento-swu').value = t.eventoId || '';
                        handleEventoChange(t.eventoId);
                    }
                    setTimeout(() => { $id('select-torneo-subevento-swu').value = t.subeventoId || ''; }, 300);
                }
            }
            new bootstrap.Modal($id('modal-torneo-swu')).show();
        }

        async function handleTorneoFormSubmit(e) {
            e.preventDefault();
            const id = $id('torneo-swu-id').value;
            const dt = $id('torneo-swu-fecha').value;
            const [fecha, hora] = dt ? dt.split('T') : ['', ''];
            const esLiga = $id('torneo-swu-es-liga').checked;
            let finalEventoId = null;
            let finalLigaId = null;
            if (esLiga) {
                finalLigaId = $id('select-torneo-liga-swu').value || null;
                const ligaObj = ligasCache.find(l => l.id === finalLigaId);
                finalEventoId = ligaObj ? ligaObj.eventoId : null;
            } else {
                finalEventoId = $id('select-torneo-evento-swu').value || null;
            }
            const baseData = {
                nombre: $id('torneo-swu-nombre').value.trim(),
                fecha: fecha,
                hora: hora || '',
                juego: JUEGO,
                ...
                subeventoId: $id('select-torneo-subevento-swu').value || null,
            };
            ...
            try {
                if (id) {
                    ...
                    if (window.auditar) window.auditar('torneos', 'editar', 'Torneo editado en Star Wars Unlimited');
                    ...
                    if (window.auditar) window.auditar('torneos', 'crear', 'Torneo creado en Star Wars Unlimited');
                    showAlert('Torneo creado.', 'success');
                }
                bootstrap.Modal.getInstance($id('modal-torneo-swu'))?.hide();
                await loadInitialData();
            } catch (e) {
                showAlert('Error: ' + e.message, 'danger');
            }
        ...
        async function handleIniciarTorneo(torneoId) {
            ...
            try {
                ...
                if (window.auditar) window.auditar('torneos', 'iniciar', 'Torneo iniciado en Star Wars Unlimited');
                await loadInitialData();
        ...
        async function handleEliminarTorneo(torneoId) {
            ...
            try {
                ...
                if (window.auditar) window.auditar('torneos', 'eliminar', 'Torneo eliminado de Star Wars Unlimited');
                await loadInitialData();
        ...
        async function handleFinalizarTorneo() {
            ...
            try {
                ...
                if (window.auditar) window.auditar('torneos', 'finalizar', 'Torneo finalizado en Star Wars Unlimited');
                await loadInitialData();
                const updated = torneosCache.find(t => t.id === torneo.id);
                if (updated) { selectedTournament = updated; openTournamentManagement(updated.id); }
        ...
        function renderLigaSelectors() {
            const selects = ['filter-liga-swu', 'select-liga-swu', 'select-torneo-liga-swu'];
            selects.forEach(sid => {
                const sel = $id(sid);
                if (!sel) return;
        ...
        function renderEventSelector() {
            ['select-torneo-evento-swu', 'liga-swu-evento'].forEach(sid => {
                const sel = $id(sid);
        ...
        function renderLigaClasificacion(ligaId) {
            const tbody = $id('tabla-clasificacion-liga-swu');
            if (!tbody) return;
            if (!ligaId) ligaId = $id('select-liga-swu')?.value || '';
            if (!ligaId) {
        ...
        async function handleLigaFormSubmit(e) {
            e.preventDefault();
            const data = {
                nombre: $id('liga-swu-nombre').value.trim(),
                eventoId: $id('liga-swu-evento').value,
                juego: JUEGO,
                activa: true,
            ...
            try {
                ...
                bootstrap.Modal.getInstance($id('modal-liga-swu'))?.hide();
                await loadInitialData();
        ...
        function updateEliminarLigaButton() {
            const btn = $id('btn-eliminar-liga-swu');
            const sel = $id('select-liga-swu');
            if (btn && sel) btn.disabled = !sel.value;
        }

        async function handleEliminarLiga() {
            const id = $id('select-liga-swu')?.value;
        ...
        function populateDatalist() {
            const dl = $id('datalist-usuarios-swu');
            if (!dl) return;
...
const num = (torneo.jugadores || []).filter(j => !j.dropped).length;
            if (topSize >= num) { showAlert(`El top cut debe ser menor a ${num} jugadores.`, 'warning'); return; }

            const standings = calculateStandings(torneo);
            // Los jugadores retirados no pueden entrar en el top cut
            const top = standings.filter(s => !s.dropped).slice(0, topSize);
        ...
        function setupEventListeners() {
            // Filter changes
            $id('filter-liga-swu')?.addEventListener('change', (e) => renderTournamentsList(e.target.value));
            $id('filter-show-finalized-swu')?.addEventListener('change', () => renderTournamentsList($id('filter-liga-swu')?.value));

            // Liga selector
            $id('select-liga-swu')?.addEventListener('change', (e) => {
                renderLigaClasificacion(e.target.value);
                updateEliminarLigaButton();
            });

            // Es liga toggle
            $id('torneo-swu-es-liga')?.addEventListener('change', (e) => {
                const isLiga = e.target.checked;
                $id('liga-bloque-swu').style.display = isLiga ? 'block' : 'none';
                $id('independiente-bloque-swu').style.display = isLiga ? 'none' : 'block';
                if (isLiga) {
                    $id('select-torneo-evento-swu').value = '';
                    handleTorneoLigaChange($id('select-torneo-liga-swu').value);
                } else {
                    $id('select-torneo-liga-swu').value = '';
                    handleEventoChange($id('select-torneo-evento-swu').value);
                }
            });

            // Liga change populates subeventos via the liga's linked evento
            $id('select-torneo-liga-swu')?.addEventListener('change', (e) => {
                handleTorneoLigaChange(e.target.value);
            });

            // Evento change for subevento cascade
            $id('select-torneo-evento-swu')?.addEventListener('change', (e) => handleEventoChange(e.target.value));

            // Buttons
            $id('btn-nuevo-torneo-swu')?.addEventListener('click', () => openTournamentModal());
            $id('btn-volver-lista-swu')?.addEventListener('click', () => {
                selectedTournament = null;
                $id('gestion-torneo-swu-container').style.display = 'none';
                $id('lista-torneos-swu-container').style.display = 'block';
                renderTournamentsList($id('filter-liga-swu')?.value);
            });
            $id('btn-generar-ronda-swu')?.addEventListener('click', handleGenerateRound);
            $id('btn-deshacer-ronda-swu')?.addEventListener('click', handleUndoRound);
            $id('btn-deshacer-topcut-swu')?.addEventListener('click', handleUndoTopCut);
            $id('btn-finalizar-torneo-swu')?.addEventListener('click', handleFinalizarTorneo);
            $id('sorteo-lanzar-swu')?.addEventListener('click', lanzarSorteo);
            $id('btn-iniciar-torneo-swu-gestion')?.addEventListener('click', () => {
                if (selectedTournament) handleIniciarTorneo(selectedTournament.id);
            });
            $id('btn-gestionar-jugadores-swu')?.addEventListener('click', () => {
                if (selectedTournament) openJugadoresModal(selectedTournament.id);
            });
            $id('btn-crear-liga-swu')?.addEventListener('click', () => {
                $id('form-liga-swu').reset();
                new bootstrap.Modal($id('modal-liga-swu')).show();
            });
            $id('btn-eliminar-liga-swu')?.addEventListener('click', handleEliminarLiga);
            $id('btn-confirmar-fusion-swu')?.addEventListener('click', handleFusionarPuntos);
            $id('btn-generar-codigo-swu')?.addEventListener('click', () => {
                $id('input-codigo-invitado-swu').value = generateLeagueCode();
            });

            // Forms
            $id('form-torneo-swu')?.addEventListener('submit', handleTorneoFormSubmit);
            $id('form-liga-swu')?.addEventListener('submit', handleLigaFormSubmit);
            $id('form-resultados-swu')?.addEventListener('submit', handleResultadosSubmit);

            // Player management
            $id('btn-agregar-usuario-swu')?.addEventListener('click', handleAgregarUsuario);
            $id('input-buscar-usuario-swu')?.addEventListener('keydown', (e) => { if (e.key === 'Enter') { e.preventDefault(); handleAgregarUsuario(); } });
            $id('btn-agregar-invitado-swu')?.addEventListener('click', handleAgregarInvitado);
            $id('btn-confirmar-jugadores-swu')?.addEventListener('click', handleConfirmJugadores);

            // Import inscritos
            $id('btn-importar-inscritos-swu')?.addEventListener('click', handleImportarInscritosJornada);

            // Filter current players
            const filterActuales = document.getElementById('filter-jugadores-actuales-swu');
            if (filterActuales) {
                filterActuales.addEventListener('input', () => renderTempJugadoresList());
            }

            // Filter previous players
            const filterPrevios = document.getElementById('filter-jugadores-previos-swu');
            if (filterPrevios) {
            ...
            $(document).on('click', (e) => {
                ...
                if (target.is('.btn-gestionar-swu')) openTournamentManagement(id);
                if (target.is('.btn-iniciar-swu')) handleIniciarTorneo(id);
                if (target.is('.btn-editar-torneo-swu')) openTournamentModal(id);
                if (target.is('.btn-eliminar-torneo-swu')) handleEliminarTorneo(id);
                if (target.is('.btn-add-players-torneo-swu')) openJugadoresModal(id);
                if (target.is('.btn-ver-detalles-torneo-swu')) openTournamentManagement(id);
                if (target.is('.btn-quitar-jugador-swu')) handleRemoveJugadorTemp(id);
                if (target.is('.btn-drop-jugador')) handleDropJugador(id);
                if (target.is('.btn-reincorporar-jugador')) handleReincorporarJugador(id);
                if (target.is('.btn-resultados-swu')) {
                    openMesaResultsModal(parseInt(target.data('ronda')), parseInt(target.data('mesa')), false);
                }
                if (target.is('#btn-generar-topcut-swu')) handleGenerateTopCut();
                if (target.is('#btn-avanzar-topcut-swu')) handleAvanzarTopCut();
            });

            // Fusion modal - populate dropdowns
            $id('btn-fusionar-puntos-swu')?.addEventListener('click', () => {
                const ligaId = $id('select-liga-swu')?.value;
                ...
                const selInv = $id('select-invitado-fusion-swu');
        ...
        async function handleFusionarPuntos() {
            const ligaId = $id('select-liga-swu')?.value;
            const invitedId = $id('select-invitado-fusion-swu')?.value;
            const userId = $id('select-usuario-fusion-swu')?.value;
            ...
            try {
                ...
                bootstrap.Modal.getInstance($id('modal-fusion-swu'))?.hide();
                await loadInitialData();
        ...
        function handleTorneoLigaChange(ligaId) {
            const sel = $id('select-torneo-subevento-swu');
            if (!sel) return;
            if (!ligaId) {
